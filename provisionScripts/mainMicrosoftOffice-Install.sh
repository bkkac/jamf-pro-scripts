...
# Variables
applicationTitle="Microsoft Office" 
customTrigger="provision_microsoftOffice" # The name of the cuntom Trigger for the Provisiong policy:  provision_firefox
appPath="" ## Leave blank if /Applacations
appIcon="/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app/Contents/Resources/AppIcon.icns" ## Find the icon name in: "/"$appPath"/"$applicationTitle".app/Contents/Resources/
...
#
version=$(mdls -name kMDItemVersion /"$appPath"/Microsoft\ Word.app | cut -c 19- | rev | cut -c 2- | rev)
...
if [[ $version == *"could not find"* ]]; then
	...
	killAll jamfHelper
	"$jamfHelperPath"  -startlaunchd -windowType hud -title "$title" -heading "Installed" -description "$applicationTitle version: $version" -icon "$appIcon" &
	sleep 5
    killAll jamfHelper
fi
/bin/echo >> $log
