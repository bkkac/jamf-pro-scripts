...
#
# Var for the icon
icon="/Applications/Utilities/Adobe Flash Player Install Manager.app/Contents/Resources/FlashPlayerInstaller.icns"
#

sudo -u $(ls -l /dev/console | awk '{print $3}') "$jamfHelper" -windowType hud -title "$the_time" -icon "$icon"  -heading "Flash Player" -description "Checking for updates..." >&- &
#
# ------------
...
wait 2>/dev/null
#
sudo -u $(ls -l /dev/console | awk '{print $3}') "$jamfHelper" -windowType hud -title "$the_time" -icon "$icon" -timeout 5 -heading "Flash Player" -description "Checking for updates... DONE" >&-
#
exit 0