...
if [[ ${version} != *Trnsfer* ]]; then
	...
	killAll jamfHelper
	"$jamfHelperPath"  -startlaunchd -windowType hud -title "$title" -heading "Installed" -description "$masterSlidesNumber $applicationTitle Installed" -icon "$appIcon" &
	sleep 5
    killAll jamfHelper
fi
/bin/echo >> $log
